...
const PrivateMessagingPage: React.FC = () => {
  ...
  return (
    <Layout variant="dark">
      ...
      {/* List of Conversation Cards (Using ConversationPreviewCard) */}
      <div className="bg-purple-900 bg-opacity-40 rounded-3xl p-6 shadow-xl border border-purple-800 border-opacity-50 space-y-4">
        {conversations.length === 0 && (
          <p className="text-white">
            No conversations yet. Click compose to start a new one.
          </p>
        )}

        {conversations.map((c) => {
          const partner = c.participants?.find(
            (p: any) => p.email !== undefined
          ); // best-effort
          const name = partner
            ? partner.firstName
          ...
          const updatedAt =
            c.messages && c.messages.length > 0
              ? c.messages[c.messages.length - 1].sentAt
              : "";
          return (
