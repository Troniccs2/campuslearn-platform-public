...
// NOTE: useAuth is not needed here as this is a public page
// NOTE: BackButton is replaced with a custom button
...
// Keep the same system instruction
...
const PublicAICopilotPage: React.FC = () => {
  const navigate = useNavigate();
  const [input, setInput] = useState("");
  const [isLoading, setIsLoading] = useState(false);
  const [messages, setMessages] = useState<Message[]>([]);
  // Removed useAuth import and user variable
  ...
  const handleBack = () => {
    // Navigates to the previous page in history (e.g., the About Page)
    navigate(-1);
  };
  ...
  return (
    <Layout variant="dark">
      <div className="flex justify-between items-center mb-6">
        ...
        {/* Removed "Escalate to Tutor" button */}
      </div>
...
export default PublicAICopilotPage;
